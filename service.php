<?php include 'includes/header.php'; ?>
<div class="banner-article-detail-section">
    <div class="container">
        <div class="breadcrumbs">
            <nav aria-label="breadcrumb">
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="/"><img src="/assets/images/icons/home.png" alt="Главная"> </a></li>
                    <li>/</li>
                    <li class="breadcrumb-item active" aria-current="page">Лечение алкоголизма методом Двоженко</li>
                </ul>
            </nav>
        </div>

        <div class="article-detail-content">
            <div class="content-left">
                <div class="article-detail-content__title">
                    <h1>Лечение алкоголизма методом Двоженко</h1>
                    <div class="article-detail-content__price">
                        <span>Стоимость лечения:</span>
                        <span>от 5000 ₽</span>
                    </div>
                    <p>
                        Наши специализированные программы лечения алкоголизма предлагают комплексный подход, включающий медицинскую поддержку, психологическую терапию и индивидуальные планы реабилитации
                    </p>
                    <a href="tel:8 (888) 999-99-99">+7(999) 999-99-99</a>
                </div>

                <div class="article-detail-info service">
                    <div class="article-detail-info-item">
                        <button class="btn button">Бесплатная консультация</button>
                    </div>
                    <div class="article-detail-info-item price">
                        <button class="btn button">Смотреть прайс</button>
                    </div>
                </div>
            </div>
            <div class="content-right">
                <picture>
                    <source srcset="/assets/images/photos/image-service.webp" type="image/webp">
                    <img
                        src="/assets/images/photos/image-service.png"
                        alt="Медицинский процесс" />
                </picture>
            </div>
        </div>
    </div>
</div>


<div class="article-detail-section">
    <div class="container">
        <div class="article-detail service-section">
            <div class="part-block">
                <div class="article-accordion-wrapper">
                    <div class="article-accordion-header open" id="contents-header">
                        Содержание статьи
                        <img src="./assets/images/icons/up-arrow.png" alt="Стрелка" class="arrow-icon" />
                    </div>
                    <div class="article-accordion-content open" id="contents-content">
                        <div class="article-accordion" id="articleAccordion"></div>
                    </div>
                </div>
                <h2>
                    H2 Краткий заголовок
                </h2>
                <h3>H3 Длинный заголовок для статьи о лечении</h3>
                <p>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Neque dolor cumque aut quaerat dignissimos natus ipsa molestiae repellat ipsam, quo magni, nihil facere est voluptatibus nesciunt quis pariatur ipsum sapiente?</p>
                <h4>H4 Длинный заголовок для статьи о лечении</h4>
                <h5>H5 Длинный заголовок для статьи о лечении</h5>
                <h2>H2 Краткий заголовок</h2>
                <h3>
                    H3 Длинный заголовок для статьи о лечении
                </h3>
                <p>
                    Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                    Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                </p>
                <p>
                    Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                    Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                </p>
                <p>
                    Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                    Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                </p>
                <ol>
                    <li>Проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека.</li>
                    <li>Проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека.</li>
                    <li>Проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека.</li>
                    <li>Проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека.</li>
                    <li>Проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека.</li>
                </ol>

                <blockquote class="blockquote-service">
                    <p class="quote-service">Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека.</p>
                    <div class="arrow-down"></div>
                    <div class="author-info-service">
                        <img src="/assets/images/photos/doctor.png" alt="Фото врача" class="author-photo">
                        <div class="author-details">
                            <cite class="author-name-service">Иванов Иван Иванович</cite>
                            <span class="author-position-service">Главный врач, психиатр-нарколог</span>
                        </div>
                    </div>
                </blockquote>



                <h3>
                    H3 Длинный заголовок для статьи о лечении
                </h3>
                <p>
                    Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                    Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                </p>
                <p>
                    Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                    Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                </p>

                <div class="attention-part">
                    <img src="/assets/images/icons/attention.png" alt="Внимание" />
                    <p>Не занимайтесь самолечением, обращайтесь за помощью к квалифицированным специалистам клиники «Призма»</p>
                </div>
                <div class="symptom-block">
                    <div class="left-side">
                        <div class="symptom-header">
                            <img src="/assets/images/icons/call.png" alt="Иконка симптомов" class="symptom-icon">
                            <span class="symptom-title">Заметили похожие симптомы?</span>
                        </div>
                        <ul class="symptom-list">
                            <li>Тошнота;</li>
                            <li>Головокружение;</li>
                            <li>Тремор;</li>
                            <li>Высокая температура;</li>
                        </ul>
                    </div>
                    <div class="right-side">
                        <form action="#" method="POST" class="form-container">
                            <div class="form-header">
                                <span class="form-title">Оставьте заявку!</span>
                                <p class="form-subtitle">Мы можем вам помочь</p>
                            </div>
                            <div class="input-group">
                                <input type="tel" placeholder="+7 (___) ___ __-__" required>
                                <button type="submit">Отправить</button>
                            </div>
                            <label class="checkbox-container">
                                <input type="checkbox" required>
                                <span>Отправляя данные вы соглашаетесь c <a href="/policy"> политикой конфиденциальности</a></span>
                            </label>
                        </form>
                    </div>
                </div>
                <h3>
                    H3 Длинный заголовок для статьи о лечении
                </h3>
                <p>
                    Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                    Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                </p>
                <ul>
                    <li>Проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека.</li>
                    <li>Проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека.</li>
                    <li>Проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека.</li>
                    <li>Проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека.</li>
                    <li>Проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека.</li>
                </ul>
                <picture>
                    <source srcset="/assets/images/photos/photo-doctor.webp" type="image/webp">
                    <img
                        src="/assets/images/photos/photo-doctor.png"
                        alt="Фото врача" />
                </picture>
                <div class="table-container">
                    <table>
                        <thead>
                            <tr>
                                <th>Заголовок столбца 1</th>
                                <th>Заголовок столбца 2</th>
                                <th>Заголовок столбца 3</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>Положительным эффектом, компьютеры оказывают негативное влияние на человека.</td>
                                <td>Положительным эффектом, компьютеры оказывают негативное влияние на человека.</td>
                                <td>Положительным эффектом, компьютеры оказывают негативное влияние на человека.</td>
                            </tr>
                            <tr>
                                <td>эффект, компьютеры оказывают негативное влияние на человека.</td>
                                <td>эффект, компьютеры оказывают негативное влияние на человека.</td>
                                <td>эффект, компьютеры оказывают негативное влияние на человека.</td>
                            </tr>
                            <tr>
                                <td>Но проблема в том, что вместе с положительным эффектом, компьютеры</td>
                                <td>Но проблема в том, что вместе с положительным эффектом, компьютеры</td>
                                <td>Но проблема в том, что вместе с положительным эффектом, компьютеры</td>
                            </tr>
                            <tr>
                                <td>Но проблема в том, что вместе с положительным эффектом, компьютеры</td>
                                <td>Но проблема в том, что вместе с положительным эффектом, компьютеры</td>
                                <td>Но проблема в том, что вместе с положительным эффектом, компьютеры</td>
                            </tr>
                        </tbody>
                    </table>
                </div>

                <div class="faq-section">
                    <div class="faq-wrapper">
                        <div class="faq-upper-block">
                            <div class="faq-title-main">Часто задаваемые вопросы</div>
                            <div class="btn_consultation button">Задать вопрос</div>
                        </div>
                        <div class="accordion">
                            <div class="accordion-item">
                                <div class="upper-block open">
                                    <div class="accordion-title">В какое время можно обратиться в кликнику</div>
                                    <div class="accordion-arrow open">
                                        <img src="./assets/images/icons/up-arrow.png" />
                                    </div>
                                </div>
                                <div class="accordion-content open">
                                    <p>Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                                        Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. </p>
                                    <div class="respondent">
                                        <picture>
                                            <source srcset="./assets/images/photos/doctor.webp" type="image/webp">
                                            <img
                                                src="./assets/images/photos/doctor.png"
                                                alt="Фото врача"
                                                class="slide-image" />
                                        </picture>
                                        <div class="respondent-info">
                                            <div class="respondent-placeholder">На вопрос ответил врач:</div>
                                            <div class="respondent-name">Королькова Ольга Николаевна</div>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div class="accordion-item">
                                <div class="upper-block">
                                    <div class="accordion-title">В какое время можно обратиться в кликнику</div>
                                    <div class="accordion-arrow">
                                        <img src="./assets/images/icons/up-arrow.png" />
                                    </div>
                                </div>
                                <div class="accordion-content">
                                    <p>Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                                        Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. </p>
                                    <div class="respondent">
                                        <picture>
                                            <source srcset="./assets/images/photos/doctor.webp" type="image/webp">
                                            <img
                                                src="./assets/images/photos/doctor.png"
                                                alt="Фото врача"
                                                class="slide-image" />
                                        </picture>
                                        <div class="respondent-info">
                                            <div class="respondent-placeholder">На вопрос ответил врач:</div>
                                            <div class="respondent-name">Королькова Ольга Николаевна</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <div class="upper-block">
                                    <div class="accordion-title">В какое время можно обратиться в кликнику</div>
                                    <div class="accordion-arrow">
                                        <img src="./assets/images/icons/up-arrow.png" />
                                    </div>
                                </div>
                                <div class="accordion-content">
                                    <p>Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                                        Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. </p>
                                    <div class="respondent">
                                        <picture>
                                            <source srcset="./assets/images/photos/doctor.webp" type="image/webp">
                                            <img
                                                src="./assets/images/photos/doctor.png"
                                                alt="Фото врача"
                                                class="slide-image" />
                                        </picture>
                                        <div class="respondent-info">
                                            <div class="respondent-placeholder">На вопрос ответил врач:</div>
                                            <div class="respondent-name">Королькова Ольга Николаевна</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <div class="upper-block">
                                    <div class="accordion-title">В какое время можно обратиться в кликнику</div>
                                    <div class="accordion-arrow">
                                        <img src="./assets/images/icons/up-arrow.png" />
                                    </div>
                                </div>
                                <div class="accordion-content">
                                    <p>Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                                        Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. </p>
                                    <div class="respondent">
                                        <picture>
                                            <source srcset="./assets/images/photos/doctor.webp" type="image/webp">
                                            <img
                                                src="./assets/images/photos/doctor.png"
                                                alt="Фото врача"
                                                class="slide-image" />
                                        </picture>
                                        <div class="respondent-info">
                                            <div class="respondent-placeholder">На вопрос ответил врач:</div>
                                            <div class="respondent-name">Королькова Ольга Николаевна</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <div class="upper-block">
                                    <div class="accordion-title">В какое время можно обратиться в кликнику</div>
                                    <div class="accordion-arrow">
                                        <img src="./assets/images/icons/up-arrow.png" />
                                    </div>
                                </div>
                                <div class="accordion-content">
                                    <p>Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. Положительным эффектом, компьютеры оказывают негативное влияние на человека.
                                        Но проблема в том, что вместе с положительным эффектом, компьютеры оказывают негативное влияние на человека. </p>
                                    <div class="respondent">
                                        <picture>
                                            <source srcset="./assets/images/photos/doctor.webp" type="image/webp">
                                            <img
                                                src="./assets/images/photos/doctor.png"
                                                alt="Фото врача"
                                                class="slide-image" />
                                        </picture>
                                        <div class="respondent-info">
                                            <div class="respondent-placeholder">На вопрос ответил врач:</div>
                                            <div class="respondent-name">Королькова Ольга Николаевна</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="literature-section">
                    <div class="literature-wrapper">
                        <div class="literature-title">
                            Используемая литаратура
                        </div>
                        <ol>
                            <li>Розенталь Д.Э. Большой справочник по русскому языку. – М. : Издательство Оникс: Издательство «Мир и образование»</li>
                            <li>Розенталь Д.Э. Большой справочник по русскому языку. – М. : Издательство Оникс: Издательство «Мир и образование»</li>
                            <li>Розенталь Д.Э. Большой справочник по русскому языку. – М. : Издательство Оникс: Издательство «Мир и образование»</li>
                        </ol>
                    </div>
                </div>

                <div class="article-footer">
                    <div class="article-footer-wrapper">
                        <div class="doctor-article-item">
                            <img src="/assets/images/photos/doctor.png">
                            <div class="doctor-info">
                                <div class="upper-desc">Статью на странице написал врач:</div>
                                <div class="doctor-name"><a>Королькова Ольга Николаевна</a></div>
                                <div class="doctor-job">Психиатр-нарколог, психотерапевт</div>
                                <div class="date-of">Дата написания статьи: 02.06.2024</div>
                            </div>
                        </div>
                        <div class="doctor-article-item">
                            <img src="/assets/images/photos/doctor.png">
                            <div class="doctor-info">
                                <div class="upper-desc">Статью на странице проверил врач:</div>
                                <div class="doctor-name"><a>Королькова Ольга Николаевна</a></div>
                                <div class="doctor-job">Психиатр-нарколог, психотерапевт</div>
                                <div class="date-of">Дата написания статьи: 02.06.2024</div>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="article-set-rating">
                    <div class="article-set-rating-wrapper">
                        <div class="stars" id="rating-stars">
                            <div class="rating-text">Оценить: </div>
                            <span data-value="1">★</span>
                            <span data-value="2">★</span>
                            <span data-value="3">★</span>
                            <span data-value="4">★</span>
                            <span data-value="5">★</span>
                        </div>
                        <div class="share">
                            <div class="share-text">Поделиться: </div>
                            <img src="/assets/images/icons/art1.png" alt="Share Icon 1" />
                            <img src="/assets/images/icons/art2.png" alt="Share Icon 2" />
                            <img src="/assets/images/icons/art3.png" alt="Share Icon 3" />
                            <img src="/assets/images/icons/art4.png" alt="Share Icon 4" />
                        </div>
                    </div>
                </div>

                <div class="more-articles">
                    <div class="more-articles-header">
                        <div class="more-articles-title">Еще статьи по теме</div>
                        <div class="more-articles-controls">
                            <button class="more-articles-button prev">
                                <img style="width: 6px;" src="/assets/images/icons/button-prev.png">
                            </button>
                            <button class="more-articles-button next">
                                <img style="width: 6px; transform: rotateY(180deg)" src="/assets/images/icons/button-prev.png">
                            </button>
                        </div>
                    </div>
                    <div class="more-articles-wrapper swiper">
                        <div class="more-articles-container swiper-wrapper">
                            <!-- Карточка 1 -->
                            <div class="more-articles-card swiper-slide">
                                <div class="card-image">
                                    <picture>
                                        <source srcset="/assets/images/photos/article-image.webp" type="image/webp">
                                        <img src="/assets/images/photos/blog.png" alt="Article Image" />
                                    </picture>
                                    <div class="card-tags">
                                        <span class="tag date">12.09.2023</span>
                                        <span class="tag type">Алкоголь</span>
                                    </div>
                                </div>
                                <div class="card-content">
                                    <div class="card-title">Как алкоголь влияет на настроение</div>
                                    <div class="card-description">
                                        Наши специалисты обладают многолетним опытом и регулярно совершенствуют свои навыки.
                                    </div>
                                </div>
                            </div>
                            <!-- Карточка 2 -->
                            <div class="more-articles-card swiper-slide">
                                <div class="card-image">
                                    <picture>
                                        <source srcset="/assets/images/photos/article-image.webp" type="image/webp">
                                        <img src="/assets/images/photos/blog.png" alt="Article Image" />
                                    </picture>
                                    <div class="card-tags">
                                        <span class="tag date">15.09.2023</span>
                                        <span class="tag type">Здоровье</span>
                                    </div>
                                </div>
                                <div class="card-content">
                                    <div class="card-title">Как алкоголь влияет на здоровье</div>
                                    <div class="card-description">
                                        Наши специалисты обладают многолетним опытом и регулярно совершенствуют свои навыки.
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div style="position: relative" class="swiper-pagination"></div>
                    </div>
                </div>
            </div>
            <div class="right-block">
                <?php include 'includes/form-right-side.php'; ?>
                <div class="brigade-wrapper">
                    <div class="brigade">
                        <div class="brigade-title">Бригад на линии</div>
                        <div class="brigade-items">
                            <div class="brigade-item">
                                <img src="/assets/images/icons/brigade.png" alt="Бригада" />
                                <div class="brigade-item-info">
                                    <div class="count-brigade">10 бригад</div>
                                    <div class="desc-brigade">Сейчас дежурят</div>
                                </div>
                            </div>
                            <div class="brigade-item">
                                <img src="/assets/images/icons/doctor.png" alt="Врач" />
                                <div class="brigade-item-info">
                                    <div class="count-brigade">20 врачей</div>
                                    <div class="desc-brigade">В вашем городе</div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="tax-block">
                    <div class="tax-block-wrapper">
                        <div class="tax-body">
                            <div class="tax-content">
                                <div class="tax-title">Поможем оформить налоговый вычет</div>
                                <div class="tax-desc">Поможем собрать все необходимые чеки, справки и заявления.</div>
                                <button class="btn tax-btn">Узнать подробнее</button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="installment-block">
                    <div class="installment-block-wrapper">
                        <div class="installment-body">
                            <div class="installment-title">Рассрочка 0%</div>
                            <div class="installment-desc">Оформите беспроцентную рассрочку на услуги клиники, первый платеж - 0₽.</div>
                            <div class="installment-items">
                                <div class="installment-item">
                                    <div class="number-block-installment">0 %</div>
                                    <div class="desc">Рассрочка</div>
                                </div>
                                <div class="installment-item">
                                    <div class="number-block-installment">0 ₽</div>
                                    <div class="desc">Первый платеж</div>
                                </div>
                                <div class="installment-item">
                                    <div class="number-block-installment">3 / 6 / 12</div>
                                    <div class="desc">Месяцев</div>
                                </div>
                            </div>
                            <button class="btn installment-btn">Узнать подробнее</button>
                        </div>
                    </div>
                </div>

                <div class="advantages-service-block">
                    <div class="advantages-wrapper">
                        <div class="block-title">Наши преимущества</div>
                        <div class="swiper advantages-service-swiper">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide">
                                    <div class="advantages-service-card">
                                        <div class="advantages-service-card-desc">Опытные врачи с высоким стажем работы</div>
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="advantages-service-card">
                                        <div class="advantages-service-card-desc">Опытные врачи с высоким стажем работы</div>
                                    </div>
                                </div>
                            </div>
                            <div style="position: relative" class="swiper-pagination"></div>
                        </div>
                    </div>
                </div>

                <div class="professionals-service-block">
                    <div class="professionals-service-wrapper">
                        <div class="professionals-service-title">Профессиональные врачи-наркологи</div>
                        <div class="swiper professionals-service-swiper">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide">
                                    <div class="doctor-card">
                                        <div class="doctor-name">Иванов Иван Иванович</div>
                                        <div class="doctor-description">Главный врач, заведующий отделением наркологии</div>
                                        <div class="doctor-image">
                                            <picture>
                                                <source srcset="/assets/images/photos/doctor.webp" type="image/webp">
                                                <img
                                                    src="/assets/images/photos/doctor.png"
                                                    alt="Фото врача"
                                                    class="slide-image" />
                                            </picture>
                                        </div>
                                        <button class="btn_consultation button">Задать вопрос врачу</button>
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="doctor-card">
                                        <div class="doctor-name">Иванов Иван Иванович</div>
                                        <div class="doctor-description">Главный врач, заведующий отделением наркологии</div>
                                        <div class="doctor-image">
                                            <picture>
                                                <source srcset="/assets/images/photos/doctor.webp" type="image/webp">
                                                <img
                                                    src="/assets/images/photos/doctor.png"
                                                    alt="Фото врача"
                                                    class="slide-image" />
                                            </picture>
                                        </div>
                                        <button class="btn_consultation button">Задать вопрос врачу</button>
                                    </div>
                                </div>
                            </div>
                            <div style="position: relative; padding-top: 20px" class="swiper-pagination"></div>
                        </div>
                    </div>
                </div>

                <div class="licenses-service-block">
                    <div class="licenses-service-wrapper">
                        <div class="licenses-service-title">Лицензии клиники</div>
                        <div class="swiper licenses-service-swiper">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide">
                                    <a href="/assets/images/photos/license.png" data-fancybox="gallery" class="license-card">
                                        <div class="license-image">
                                            <img src="/assets/images/photos/license.png" alt="Лицензия">
                                        </div>
                                        <div class="lic-desc">Выписка Л041-01164-52/01066266 клиники Призма</div>
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/assets/images/photos/license.png" data-fancybox="gallery" class="license-card">
                                        <div class="license-image">
                                            <img src="/assets/images/photos/license.png" alt="Лицензия">
                                        </div>
                                        <div class="lic-desc">Выписка Л041-01164-52/01066266 клиники Призма</div>
                                    </a>
                                </div>
                            </div>
                            <div style="position: relative; margin-top: 10px" class="swiper-pagination"></div>
                        </div>
                    </div>
                </div>

                <div class="gallery-service-block">
                    <div class="gallery-service-wrapper">
                        <div class="gallery-service-title">Фотогалерея</div>
                        <div class="swiper gallery-service-swiper">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide">
                                    <a href="/assets/images/photos/galleryitem.png" data-fancybox="gallery">
                                        <img src="/assets/images/photos/galleryitem.png" alt="Фото" />
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/assets/images/photos/galleryitem.png" data-fancybox="gallery">
                                        <img src="/assets/images/photos/galleryitem.png" alt="Фото" />
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/assets/images/photos/galleryitem.png" data-fancybox="gallery">
                                        <img src="/assets/images/photos/galleryitem.png" alt="Фото" />
                                    </a>
                                </div>
                            </div>
                            <div style="position: relative; margin-top: 35px" class="swiper-pagination"></div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="thank-you-message" id="thankYouMessage">
        Спасибо за вашу оценку!
    </div>
</div>

<?php include 'includes/footer-map.php'; ?>
<?php include 'includes/footer.php'; ?>